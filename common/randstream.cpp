...
RandomFileStream::RandomFileStream()
    : m_cRef(0), hFile(INVALID_HANDLE_VALUE), hEvent(nullptr)
{
...
{
    auto pStream = new RandomFileStream();
...
{
    Close();
    hFile = CreateFile(lpFileName, dwDesiredAccess, dwShareMode, nullptr,
...
HRESULT RandomFileStream::Read(void* pv, ULONG cb, ULONG* pcbRead)
{
    *pcbRead = 0;
    ...
    if (!ReadFile(hFile, pv, cb, pcbRead, nullptr))
...
HRESULT RandomFileStream::Write(const void* pv, ULONG cb, ULONG* pcbWritten)
{
    *pcbWritten = 0;
    ...
    if (!WriteFile(hFile, pv, cb, pcbWritten, nullptr))
        return HRESULT_FROM_WIN32(GetLastError());
...
HRESULT RandomFileStream::SetSize(ULARGE_INTEGER libNewSize)
{
    ...
    HRESULT hr = Seek(size, STREAM_SEEK_SET, nullptr);
    if (FAILED(hr))
        return hr;
...
STDMETHODIMP RandomFileStream::Tell(ULARGE_INTEGER* pPosition)
{
    ...
    pPosition->LowPart = dwPtrLow;
    pPosition->HighPart = high;
